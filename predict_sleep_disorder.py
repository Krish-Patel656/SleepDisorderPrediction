...
def predict_sleep_disorder():
    ...
    user_input_df = pd.DataFrame([[stress_level, sleep_duration, quality_sleep]], columns=["Stress Level", "Sleep Duration", "Quality of Sleep"])
    ...
    if len(prediction_prob) == 2:
        if prediction[0] == 0:
            ...
            print(f"Predicted Sleep Disorder: {prediction_label}")
            print(f"Prediction Probability: {prediction_prob[0]:.2f}")
        else:
            prediction_label = "Sleep Apnea"
            print(f"Predicted Sleep Disorder: {prediction_label}")
            print(f"Prediction Probability: {prediction_prob[1]:.2f}")
    else:
        print("Model is predicting only one class. Please check the training data or model performance.")
